  ...
  with ChiselScalatestTester
{
  ...
  it should "test r/w" in {
    test(new Regfile) { c =>
      ...
      for(i <- 0 until TEST_SIZE) {
        println(s"${i}. 测试数据：   addr1=${addr1(i)}  data1=${data1(i)}")
        c.io.wData.poke(data1(i).U)
        c.io.wAddr.poke(addr1(i).U)
        c.io.wen.poke(true.B)
        c.io.r1Addr.poke(addr1(i).U)
        val temp1 = c.io.r1Data.peek

        println(s"${i}. 时钟到来前：")
        println(s"${i}. 寄存器：     r1Addr=${c.io.r1Addr.peek}  r1Data=${c.io.r1Data.peek}")
        c.io.r1Data.expect(temp1)
        ...
        println(s"${i}. 测试数据：   addr2=${addr2(i)}  data2=${data2(i)}")
        c.io.wData.poke(data2(i).U)
        c.io.wAddr.poke(addr2(i).U)
        c.io.wen.poke(true.B)
        c.io.r2Addr.poke(addr2(i).U)
        val temp2 = c.io.r2Data.peek

        println(s"${i}. 时钟到来前：")
        println(s"${i}. 寄存器：     r2addr=${c.io.r2Addr.peek}  r2data=${c.io.r2Data.peek}")
        c.io.r2Data.expect(temp2)
        ...
        // c.io.wAddr.poke(addr1(i).U)
        // c.io.wen.poke(true.B)
        // println(s"${i}. 写入： 写入地址=${add_out(i) % (1<<16)} 模块结果 io.out=${c.io.out.peek}")
        // c.clock.step()
