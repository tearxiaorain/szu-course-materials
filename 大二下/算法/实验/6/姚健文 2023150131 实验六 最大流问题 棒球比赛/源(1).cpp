...
public:
    EdmondsKarp(int size) : n(size), graph(size, vector<int>(size, 0)) {}
...
int buildAndComputeFlow(const map<string, TeamInfo>& allTeams, const string& teamToCheck, int& totalPossibleFlow) {
    ...
    for (int i = 0; i < k; ++i) {
        for (int j = i + 1; j < k; ++j) {
            ...
            ek.addEdge(source, matchNodeMap[matchName], capacity);
    ...
    for (auto matchIt = matchNodeMap.begin(); matchIt != matchNodeMap.end(); ++matchIt) {
        ...
        ek.addEdge(matchNode, teamNodeMap[t1], 10000);
        ek.addEdge(matchNode, teamNodeMap[t2], 10000);
    }
    ...
    for (auto teamIt = teamNodeMap.begin(); teamIt != teamNodeMap.end(); ++teamIt) {
        ...
        ek.addEdge(teamNode, sink, capacity);
        //cout << "  " << team << " 到汇点容量 = " << capacity << " (剩余 " << info.wins << " 胜场)" << endl;
    }

    // 计算最大流
    int maxFlow = ek.computeMaxFlow(source, sink);
...
int main() {
    ...
    for (auto it = teams.begin(); it != teams.end(); ++it) {
        ...
        if (isEliminated) {
            cout << "【EK算法最大流判断】已被淘汰（最大流 " << maxFlow
                << " < 总流量 " << totalPossibleFlow << "）" << endl;
        }
        else {
            cout << "【EK算法最大流判断】未被淘汰（最大流 = 总流量 = " << maxFlow << "）" << endl;
        }
